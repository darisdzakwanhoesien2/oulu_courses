...
def cornersHeuristic(state, problem):
    ...
    # Compute the minimum distance to visit all remaining corners
    total_distance = 0
    position = current_position
    while corners:
        distances = [(util.manhattanDistance(position, corner), corner) for corner in corners]
...
def foodHeuristic(state, problem):
    ...
    # Compute the farthest distance to any food dot
    max_distance = max(util.manhattanDistance(current_position, food) for food in food_list)
    return max_distance
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        return self.food[x][y]  # Return True if there's food at the current position
