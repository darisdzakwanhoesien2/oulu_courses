...
Screenshot 1 — Front-left perspective showing the tetrahedron rotated about its local X axis. Vertex colors are interpolated across faces; this view highlights the green → magenta gradient on the top and side faces and verifies the continuous rotation is applied to the model matrix. (Camera example: slight left offset, small negative yaw.)

[[Screenshot 2026-03-31 at 19.55.40]]
Screenshot 2 — Lower-right viewpoint showing the underside of the tetrahedron and stronger color blending along the lower edges. The camera has been moved down and right to emphasize depth and the interpolation across the bottom edges. This view demonstrates keyboard movement (W/S/A/D/E/R) combined with mouse-look to inspect occluded faces.

[[Screenshot 2026-03-31 at 19.55.58]]
Screenshot 3 — Right-above viewpoint showing a rotated pose where three faces are visible simultaneously. This angle highlights the object silhouette and the smooth RGB interpolation across adjacent faces; useful for verifying vertex-color attribution and correct face ordering when using triangle primitives. (Camera example: right offset, small positive yaw.)

Notes for reproducing the views
- Environment: macOS, Python 3.10, Homebrew FreeGLUT.
- Dependencies: PyOpenGL, PyOpenGL_accelerate, PyGLM.
- Run from the project folder: python3 main.py
- Controls: mouse drag = Euler-angle look (yaw/pitch); W/S/A/D/E/R = move forward/back/left/right/up/down.